...
summarize_TxTl <- function(data_frame,
  ...
  data_frame %>%
    ...
    dplyr::summarise(max_signal_per_protein = max({{mean_RFUs}}),
                     ...
                     confidence_interval_95_at_max = stats::qnorm(0.975)*stnd_dev_at_max/(sqrt(3)),
                     logic = dplyr::if_else({{mean_RFUs}}[minutes == minutes[1]] < {{mean_RFUs}}[minutes == minutes[3]], "true", "false")) -> TxTlSum
  return(TxTlSum)
}
